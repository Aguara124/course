...
AssignmentTabAssistant::AssignmentTabAssistant(const QString &assistantLogin, QWidget *parent)
    : QWidget{parent}
...
void AssignmentTabAssistant::setupTabLayout()
{
    ...
    connect(addButton, &QAbstractButton::clicked, [=] {
        ...
        assignment->setId(0);
        assignment->setExaminerName(assistantLogin);
    ...
    GetAssistant *getAssistant = new GetAssistant(assistantLogin);
    connect(getAssistant, &GetAssistant::result, [=](const QList<std::shared_ptr<Assistant>> &result) {
        assistant = result.at(0);
    });
    getAssistant->start();

    GetAssignments *getAssignments = new GetAssignments(assistantLogin, true);
    connect(getAssignments, &GetAssignments::result, this, &AssignmentTabAssistant::updateAssignments);
    getAssignments->start();
}

void AssignmentTabAssistant::updateAssignments(const QList<std::shared_ptr<Assignment>> &result)
{
    ...
    for (int i = 0; i < assignmentCount; i++) {
        ...
        Assistant *pAssistant = assistant.get();
...
QWidget *AssignmentTabAssistant::setupFilterBox()
{
...
QWidget *AssignmentTabAssistant::setupAssignmentBox()
{
    QFont font("Roboto", 12, QFont::Normal);
...
QWidget *AssignmentTabAssistant::setupAssignmentListBox()
{
    QFont font("Roboto", 12, QFont::Normal);
...
bool AssignmentTabAssistant::eventFilter(QObject *watched, QEvent *event)
{
...
void AssignmentTabAssistant::addHeaderField(QVBoxLayout *headerLayout,
...
QStringList AssignmentTabAssistant::getAvailableGroups()
{
    QStringList groups;

    QString group = QString("%1-%2")
                        .arg(assistant->getGroup())
                        .arg(assistant->getSubGroup());
...
QStringList AssignmentTabAssistant::getAvailableSubjects()
{
    QStringList subjects;
    subjects.append(assistant->getSubject());

    return subjects;
}
void AssignmentTabAssistant::selectFile() {
...
void AssignmentTabAssistant::buildImageLayout(QVBoxLayout *parentLayout, Assignment *pAssignment)
{
    ...
    connect(pOtherFile, &QAbstractButton::clicked, this, &AssignmentTabAssistant::selectFile);

    QtMaterialRaisedButton *pSaveFile = new QtMaterialRaisedButton(this);
    pSaveFile->setBackgroundColor(QColor(204, 204, 204));
    pSaveFile->setForegroundColor(QColor(89, 89, 89));
    ...
    pOpenFile->setForegroundColor(QColor(89, 89, 89));
...
void AssignmentTabAssistant::showAssignment(QVBoxLayout *pLayout, Assignment *pAssignment)
{
    if (pLayout == nullptr) {
        return;
    }
    ...
    nameOfAssigner->setText(assistantLogin);
        ...
        if (m_pCurrentRequest->getId() == 0) {
            if (assignmentRepository->addAssignment(*m_pCurrentRequest)) {
                ...
                connect(getAssignments,
                        ...
                        this,
                        &AssignmentTabAssistant::resetAssignmentsSelection);
                getAssignments->start();
            } else {
    ...
    columnName->setInkColor(QColor(160, 198, 232));
...
void AssignmentTabAssistant::resetAssignmentsSelection(const QList<std::shared_ptr<Assignment>> &result)
{
    if (result.size()) {
        int maxId = -1;
...
bool AssignmentTabAssistant::isAssignmentExpired(Assignment *pAssignment)
{
    return pAssignment->getDeadLine().isNull() || !pAssignment->getDeadLine().isValid()
           || pAssignment->getDeadLine() < QDateTime::currentDateTime();
}

QByteArray AssignmentTabAssistant::getGradeTable()
{
...
QByteArray AssignmentTabAssistant::generateGradeTable(int group, int subGroup)
{
...
QTableWidget *AssignmentTabAssistant::setupGradeTable(Assignment *pAssignment)
{
...
void AssignmentTabAssistant::loadGradeTableToWidget(QTableWidget *tableWidget, Assignment *pAssignment)
{
...
void AssignmentTabAssistant::addTableItem(
...
void AssignmentTabAssistant::addImageToLayout(QVBoxLayout *imageLayout, const QImage &image)
{
...
void AssignmentTabAssistant::adjustGradeTableGeometry()
{
