...
ConsultationTab::ConsultationTab(const QString &teacherLogin, QWidget *parent)
    : QWidget{parent}
...
QWidget *ConsultationTab::setupFilterBox()
{
...
QWidget *ConsultationTab::setupRequestBox()
{
    QFont font("Roboto", 12, QFont::Normal);
...
QWidget *ConsultationTab::setupRequestListBox()
{
    QFont font("Roboto", 12, QFont::Normal);
...
bool ConsultationTab::eventFilter(QObject *watched, QEvent *event)
{
...
void ConsultationTab::setupTabLayout()
{
    ...
    GetConsultationRequest *consultationRequest = new GetConsultationRequest(teacherLogin, this);
...
Student *ConsultationTab::getStudent(const QString &studentLogin)
{
...
void ConsultationTab::updateRequests(const QString &studentLogin)
{
...
void ConsultationTab::showRequest(Student *pStudent, Consultation *pConsultation)
{
    QLayoutItem *item;
    ...
    if (pConsultation->getChecked()) {
        markAsViewed->setText("Mark as non viewed");
    } else {
    ...
    connect(markAsViewed, &QAbstractButton::clicked, [=] {
        for (int i = 0; i < consultations.size(); i++) {
            if (consultations.at(i)->getId() == consultationId) {
                ...
                // we already in button state, so requeue
